...
function winresize(how)
   ...
   elseif how == "center" then
      newrect = {0.07,0.07,0.86,0.86}
   elseif how == "min_center" then
      newrect = {0.15,0.1,0.7,0.8}
   elseif how == "max" then
      newrect = hs.layout.maximized
   elseif how == "bottom_right" then
      newrect = {0.55,0.4,0.45,0.6}
...
hs.hotkey.bind({"ctrl","cmd"}, "[",  hs.fnutils.partial(winresize, "left"))
hs.hotkey.bind({"ctrl","cmd"}, "]", hs.fnutils.partial(winresize, "right"))
hs.hotkey.bind({"ctrl","cmd"}, "t",    hs.fnutils.partial(winresize, "up"))
hs.hotkey.bind({"ctrl","cmd"}, "b",  hs.fnutils.partial(winresize, "down"))

-- Center of the screen
-- hs.hotkey.bind({"ctrl", "cmd"}, "=", center)
hs.hotkey.bind({"ctrl", "cmd"}, "=", hs.fnutils.partial(winresize, "center"))
hs.hotkey.bind({"ctrl", "cmd"}, "-", hs.fnutils.partial(winresize, "min_center"))

-- Thirds of the screen
hs.hotkey.bind({"ctrl", "alt"}, "[",  left_third)
hs.hotkey.bind({"ctrl", "alt"}, "]", right_third)
hs.hotkey.bind({"ctrl", "alt"}, "t",    up_third)
hs.hotkey.bind({"ctrl", "alt"}, "b",  down_third)

-- Maximized
hs.hotkey.bind({"ctrl", "cmd"}, "K",     hs.fnutils.partial(winresize, "max"))

-- Move between screens
hs.hotkey.bind({"ctrl", "alt", "cmd"}, "[",  hs.fnutils.partial(winmovescreen, "left"))
hs.hotkey.bind({"ctrl", "alt", "cmd"}, "]", hs.fnutils.partial(winmovescreen, "right"))

-- bottom_right
hs.hotkey.bind({"ctrl","cmd"}, "m",  hs.fnutils.partial(winresize, "bottom_right"))
